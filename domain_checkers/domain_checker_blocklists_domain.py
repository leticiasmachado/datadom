...
class DomainCheckerBlocklistDomain:
    ...
    def check_domain_in_openphish(self, domain):
        return "True" if domain in self.openphish_df[0].values else "False"

    def check_domain_in_hagezi(self, domain):
        return "True" if domain in self.hagezi_df[0].values else "False"

    def check_domain_in_urlabuse(self, domain):
        return "True" if domain in self.urlabuse_df[0].values else "False"

    def check_domain_in_bambenek(self, domain):
        return "True" if domain in self.bambenek_df[0].values else "False"

    def check_domain_in_phishtank(self, domain):
        return "True" if domain in self.phishtank_df[0].values else "False"

    def check_domain_in_abusech(self, domain):
        return "True" if domain in self.abusech_df[0].values else "False"
